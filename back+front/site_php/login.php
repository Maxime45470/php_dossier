            ...
            <div class="tab-pane fade show active" id="home-tab-pane" role="tabpanel" aria-labelledby="home-tab"
                ...
                <form name="inscription" enctype="multipart/form-data" method="post" action="action.php?e=inscription">
                    ...
                    <p>
                        ...
                        <input placeholder="Prenom" type="text" name="prenom" <?php if($formulaire['prenom']){
                            echo 'value="' .$formulaire['prenom'].'"';} ?>/>
                    </p>
                    <p>
                        <label for="email"></label>
                        <input placeholder="Email" type="email" name="email" <?php if($formulaire['email']) {
                            echo 'value="' .$formulaire['email'].'"';} ?>/>
                    </p>
                    <p>
                        <label for="telephone"></label>
                        <input placeholder="Tel" type="tel" name="telephone" <?php if($formulaire['telephone']) {
                            echo 'value="' .$formulaire['telephone'].'"';} ?> />
                    </p>
                    <p>
            ...
            <div class="tab-pane fade" id="profile-tab-pane" role="tabpanel" aria-labelledby="profile-tab" tabindex="0">
                <div class="espace">
                    ...
                    <form name="connection" method="post" action="action.php?e=connection">
                        <p>
                            <label for="email"></label>
                            <input type="email" placeholder="Email" name="email" <?php if($_SESSION['email']){
                                echo 'value="' .strip_tags($_SESSION['email']).'"';} ?>/>
                        </p>
                        <p>
                            <label for="password"></label>
                            <input placeholder="Mot de passe" type="password" name="password" <?php
                                if($_COOKIE['passwd_clair']){ echo 'value="' .$_COOKIE['passwd_clair'].'"';} ?>/>
